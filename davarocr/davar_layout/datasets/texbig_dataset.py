...
class TexBiGDataset(MMLayoutDataset):
    """
    ...
    def format_results(self, results, jsonfile_prefix=None, **kwargs):
        ...
        # dict with file_name : ID
        #self.img_ids = [coco_name_2_id[per['filename']] for per in self.data_infos]
        self.img_ids = [per["id"] for per in self.data_infos]
        # list of img_ids
    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            if classwise:
                ...
                eval_results[metric] = {key: float(val) for key, val in results_per_category}
        ...
        if "bbox" in eval_results:
            bbox_res = eval_results["bbox"]
            eval_results.pop("bbox")
            for key, item in bbox_res.items():
                eval_results["bbox/" + key] = item
        if "segm" in eval_results:
            bbox_res = eval_results["segm"]
            eval_results.pop("segm")
            for key, item in bbox_res.items():
                eval_results["segm/" + key] = item
        return eval_results

    # overwrite custom loader
    def _cvt_list(self, img_info):
        ...
        for key in img_info.keys():
            tmp_dict = {}
            tmp_dict["id"] = int(key)
            tmp_dict["filename"] = img_info[key]["file_name"]
